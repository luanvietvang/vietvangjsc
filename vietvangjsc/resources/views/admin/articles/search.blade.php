                            ...
                            <table class="table table-bordered table-hover table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr class="active">
                                        ...
                                        <td>{!! $i + 1 !!}</td>
                                        <td>{!! $arts[$i]->title !!}</td>
                                        <td class="text-center">{!! Html::image('/upload/products/'.$arts[$i]->img, 'img', array( 'width' => 70, 'height' => 70 )) !!}</td>
                                        <td>{!! $arts[$i]->desc !!}</td>
                                        <td>{!! $arts[$i]->category_id !!}</td>
                                        <td>{!! $arts[$i]->menu_id !!}</td>
                                        <td>{!! $arts[$i]->created_at !!}</td>
                                        <td>
                                            {!! Html::decode(Html::link('admin/products/detail/'. $arts[$i]->id,'<button type="button" class="btn btn-xs btn-info">Info</button>')) !!}
                                            {!! Html::decode(Html::link('admin/products/edit/'. $arts[$i]->id,'<button type="button" class="btn btn-xs btn-warning">Edit</button>')) !!}
                                            {!! Html::decode(Html::link('admin/products/del/'. $arts[$i]->id,'<button type="button" class="btn btn-xs btn-danger">Del</button>')) !!}
                                        </td>
                                    </tr>
                                    @endfor
                                </tbody>
